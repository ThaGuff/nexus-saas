/**
 * NEXUS SAAS · Database v3
 * Resilient — gracefully handles missing tables (schema_v2 not yet run)
 * Settings always save to users table first
 */
...
function encrypt(t){if(!t)return null;const iv=crypto.randomBytes(16),c=crypto.createCipheriv('aes-256-cbc',Buffer.from(ENC_KEY),iv),e=Buffer.concat([c.update(t),c.final()]);return iv.toString('hex')+':'+e.toString('hex');}
function decrypt(t){if(!t)return null;try{const[ivH,eH]=t.split(':'),iv=Buffer.from(ivH,'hex'),e=Buffer.from(eH,'hex'),d=crypto.createDecipheriv('aes-256-cbc',Buffer.from(ENC_KEY),iv);return Buffer.concat([d.update(e),d.final()]).toString();}catch{return null;}}
...
function rowToUser(row){
  ...
  return{
    ...
    solanaWallet:row.solana_wallet||null,
    botState:{
...
function userToRow(d){
  const r={};
  if(d.email!=null)r.email=d.email.toLowerCase();
  if(d.passwordHash!=null)r.password_hash=d.passwordHash;
  if(d.firstName!=null)r.first_name=d.firstName;
  if(d.lastName!=null)r.last_name=d.lastName;
  if(d.plan!=null)r.plan=d.plan;
  if(d.trialEndsAt!=null)r.trial_ends_at=d.trialEndsAt;
  if(d.subscriptionId!=null)r.subscription_id=d.subscriptionId;
  if(d.customerId!=null)r.customer_id=d.customerId;
  if(d.subscriptionStatus!=null)r.subscription_status=d.subscriptionStatus;
  if(d.botEnabled!=null)r.bot_enabled=d.botEnabled;
  if(d.botMode!=null)r.bot_mode=d.botMode;
  if(d.startingBalance!=null)r.starting_balance=d.startingBalance;
  if(d.maxTradeUSD!=null)r.max_trade_usd=d.maxTradeUSD;
  if(d.stopLossPct!=null)r.stop_loss_pct=d.stopLossPct;
  if(d.takeProfitPct!=null)r.take_profit_pct=d.takeProfitPct;
  if(d.maxDrawdownPct!=null)r.max_drawdown_pct=d.maxDrawdownPct;
  if(d.maxPositionPct!=null)r.max_position_pct=d.maxPositionPct;
  if(d.leverageEnabled!=null)r.leverage_enabled=d.leverageEnabled;
  if(d.maxLeverage!=null)r.max_leverage=d.maxLeverage;
  if(d.tradingStrategy!=null)r.trading_strategy=d.tradingStrategy;
  if(d.solanaWallet!=null)r.solana_wallet=d.solanaWallet;
  return r;
}
...
// ── Bots — resilient, falls back gracefully if table missing ─────────────────
function rowToBot(row){
  ...
  return{
    ...
    portfolio:row.portfolio||{},peakValue:parseFloat(row.peak_value)||100,
    cycleCount:row.cycle_count||0,totalFees:parseFloat(row.total_fees)||0,
    status:row.status||'idle',startedAt:row.started_at,lastCycleAt:row.last_cycle_at,
    maxTradeUSD:parseFloat(row.max_trade_usd)||20,
    stopLossPct:parseFloat(row.stop_loss_pct)||0.05,
    takeProfitPct:parseFloat(row.take_profit_pct)||0.08,
    maxDrawdownPct:parseFloat(row.max_drawdown_pct)||0.20,
...
export const Bots={
  forUser:async(userId)=>{
    try{const r=await sb('GET','bots',{query:`user_id=eq.${userId}&order=created_at.asc`});return(r||[]).map(rowToBot);}
    catch(e){if(e.message.includes('42P01')||e.message.includes('does not exist'))return[];throw e;}
  },
  findById:async(id)=>{
    try{const r=await sb('GET','bots',{query:`select=*&id=eq.${id}&limit=1`});return r?.[0]?rowToBot(r[0]):null;}
    catch{return null;}
  },
  create:async(userId,data)=>{
    const row={
      ...
      max_position_pct:data.maxPositionPct||0.35,
      leverage_enabled:data.leverageEnabled||false,max_leverage:data.maxLeverage||3,
    };
  ...
  resetBot:async(id)=>{
    const bot=await Bots.findById(id);if(!bot)return;
    ...
    await Trades.deleteForBot(id).catch(()=>{});
...
export const Exchanges={
  forUser:async(userId)=>{
    try{const r=await sb('GET','exchange_connections',{query:`user_id=eq.${userId}&order=connected_at.asc`});return(r||[]).map(rowToExchange);}
    catch(e){if(e.message.includes('42P01')||e.message.includes('does not exist'))return[];throw e;}
  },
  findById:async(id)=>{
    try{const r=await sb('GET','exchange_connections',{query:`select=*&id=eq.${id}&limit=1`});return r?.[0]?rowToExchange(r[0]):null;}catch{return null;}
  },
  connect:async(userId,data)=>{
    const row={user_id:userId,exchange:data.exchange,label:data.label||'',
      ...
      mode:data.mode||'PAPER',is_active:true,permissions:data.permissions||['read','trade']};
...
function rowToTrade(r){
  return{id:r.id,type:r.type,coin:r.coin,qty:parseFloat(r.qty)||0,price:parseFloat(r.price)||0,
    gross:parseFloat(r.gross)||0,fee:parseFloat(r.fee)||0,netProceeds:parseFloat(r.net_proceeds)||0,
    pnl:r.pnl!=null?parseFloat(r.pnl):null,leverage:parseFloat(r.leverage)||1,
    strategy:r.strategy,confidence:r.confidence,signals:r.signals||[],
    reasoning:r.reasoning,source:r.source,botId:r.bot_id,ts:r.created_at};
}

export const Trades={
  forUser:async(userId,limit=200)=>{
    try{const r=await sb('GET','trades',{query:`user_id=eq.${userId}&order=created_at.desc&limit=${limit}`});return(r||[]).map(rowToTrade);}
    catch(e){if(e.message.includes('42P01'))return[];throw e;}
  },
  forBot:async(botId,limit=200)=>{
    try{const r=await sb('GET','trades',{query:`bot_id=eq.${botId}&order=created_at.desc&limit=${limit}`});return(r||[]).map(rowToTrade);}
    catch(e){if(e.message.includes('42P01'))return[];throw e;}
  },
  insert:async(userId,trade,botId=null)=>{
    try{await sb('POST','trades',{body:{user_id:userId,bot_id:botId||null,type:trade.type,coin:trade.coin,qty:trade.qty,price:trade.price,gross:trade.gross,fee:trade.fee,net_proceeds:trade.netProceeds,pnl:trade.pnl,leverage:trade.leverage||1,is_perp:trade.isPerp||false,strategy:trade.strategy,confidence:trade.confidence,signals:trade.signals||[],reasoning:trade.reasoning,source:trade.source||'RULES'},prefer:'return=minimal'});}
    catch(e){if(e.message.includes('42P01'))return;throw e;}
  },
  deleteForUser:async(userId)=>{try{await sb('DELETE','trades',{query:`user_id=eq.${userId}`,prefer:'return=minimal'});}catch{}},
  deleteForBot:async(botId)=>{try{await sb('DELETE','trades',{query:`bot_id=eq.${botId}`,prefer:'return=minimal'});}catch{}},
};
...
export const BotLogs={
  append:async(key,entry)=>{
    ...
    const logs=memLogs.get(key);logs.unshift(entry);if(logs.length>300)logs.pop();
  ...
  getRecent:async(key,limit=150)=>{
    ...
    try{const r=await sb('GET','bot_logs',{query:`user_id=eq.${key}&order=created_at.desc&limit=${limit}`});const logs=(r||[]).map(row=>({ts:row.created_at,level:row.level,msg:row.msg}));memLogs.set(key,logs);return logs;}
    catch{return[];}
  },
